...
final class RetainedEventsTests: XCTestCase {
    ...
    func testRetainedEvents() async throws {
        ...
        let subscriptionId = try await node.subscribe(topic: "test/retained", options: subscriptionOptions) { _, data in
            if let data {
    ...
    func testRetainedEventsMultipleSubscribers() async throws {
        ...
        let subscriptionId1 = try await node.subscribe(topic: "test/retained", options: subscriptionOptions) { _, data in
            if let data {
                let stringValue = try? await data.asType() as String
                XCTAssertEqual(stringValue, "retained data")
        ...
        let subscriptionId2 = try await node.subscribe(topic: "test/retained", options: subscriptionOptions) { _, data in
            if let data {
                let stringValue = try? await data.asType() as String
                XCTAssertEqual(stringValue, "retained data")
    ...
    func testRetainedEventsTTLPruning() async throws {
        ...
        let subscriptionId = try await node.subscribe(topic: "test/short", options: subscriptionOptions) { _, _ in
    ...
    func testRetainedEventsCapacityLimits() async throws {
        ...
        let subscriptionId = try await node.subscribe(topic: "test/capacity", options: subscriptionOptions) { _, data in
            if let data {
                let stringValue = try? await data.asType() as String
