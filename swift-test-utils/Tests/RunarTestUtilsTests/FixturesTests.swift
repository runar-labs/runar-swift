...
final class FixturesTests: XCTestCase {
    ...
    func testTransportOptionsPeerInfoNodeInfoCBOR() throws {
        ...
        let decoded = try CodableCBORDecoder().decode(Opts.self, from: opts)
        ...
        let np = try CodableCBORDecoder().decode(PeerInfo.self, from: p)
        ...
        let dni = try CodableCBORDecoder().decode(NodeInfo.self, from: ni)
