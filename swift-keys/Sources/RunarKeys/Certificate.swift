...
public class CertificateAuthority {
    ...
    public func signCertificateRequest(csrDer: Data, validityDays: Int) throws -> X509Certificate {
        ...
        let leafCertificate = try createLeafCertificate(
            ...
            caPrivateKey: keyPair.toECDSASigningKey(),
            csr: csr,
    ...
    private static func createSelfSignedCertificate(keyPair: ECDHKeyPair, subject: String) throws -> X509Certificate {
        let certificate = try createSelfSignedCACertificate(
            subject: subject,
            privateKey: keyPair.toECDSASigningKey(),
            publicKey: keyPair.toECDSAVerifyingKey()
        )
...
public struct CertificateRequest {
    ...
    public init(derData: Data) throws {
        csr = try CertificateSigningRequest(derEncoded: Array(derData))
    }
    ...
    public static func create(keyPair: ECDHKeyPair, subject: String, subjectAltNames _: [String] = []) throws -> Data {
...
public struct CertificateValidator {
    ...
    public func validateCertificate(_ certificate: X509Certificate) throws {
        ...
        let isTrusted = trustedCaCertificates.contains { caCert in
            certificate.issuer == caCert.issuer
        }
...
public extension X509Certificate {
    ...
    func createSecIdentity(privateKeySecKey _: SecKey) throws -> SecIdentity {
    ...
    func importToKeychain(label: String) throws -> SecCertificate {
        guard let secCertificate = toSecCertificate() else {
...
private func createSelfSignedCACertificate(
    ...
    let certificate = try Certificate(
        ...
        subject: subjectDN,
        signatureAlgorithm: .ecdsaWithSHA384,
        extensions: createCAExtensions(publicKey: publicKey),
...
private func createLeafCertificate(
    ...
    let validityDuration = TimeInterval(validityDays * 24 * 60 * 60)
    ...
    let certificate = try Certificate(
        ...
        signatureAlgorithm: .ecdsaWithSHA384,
        extensions: createEndEntityExtensions(
            publicKey: csr.publicKey,
...
private func extractSansFromCsr(_: CertificateSigningRequest) throws -> [GeneralName] { [] }
