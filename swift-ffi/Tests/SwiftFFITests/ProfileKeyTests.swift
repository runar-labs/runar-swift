...
final class ProfileKeyTests: XCTestCase {
    ...
    func testInstallProfilePublicKeyWrongManagerType() throws {
        ...
        XCTAssertThrowsError(try mobileKeys.installProfilePublicKey(testPublicKey)) { error in
            XCTAssertTrue(error is FFIError, "Should throw FFIError")
        }
    }
    ...
    func testGetProfilePublicKeyByLabelHappyPath() throws {
        ...
        // Retrieve it
        let retrievedKey = try nodeKeys.getProfilePublicKeyByLabel(label)
    ...
    func testGetProfilePublicKeyByLabelAfterInstall() throws {
        ...
        let beforeInstall = try nodeKeys.getProfilePublicKeyByLabel(label)
        ...
        // Now should exist
        let afterInstall = try nodeKeys.getProfilePublicKeyByLabel(label)
    ...
    func testGetProfilePublicKeyByLabelWrongManagerType() throws {
        ...
        XCTAssertThrowsError(try mobileKeys.getProfilePublicKeyByLabel(label)) { error in
            XCTAssertTrue(error is FFIError, "Should throw FFIError")
        }
    }
    ...
    func testGetProfilePublicKeyByLabelNotInitialized() throws {
        ...
        XCTAssertThrowsError(try uninitializedKeys.getProfilePublicKeyByLabel(label)) { error in
            XCTAssertTrue(error is FFIError, "Should throw FFIError")
        }
    }
    ...
    func testDecryptWithProfileHappyPath() throws {
        ...
        let encryptedData = try nodeKeys.encryptWithEnvelope(plaintext: testData, profileKeys: [profileKey])
        
        // Decrypt data
        let decryptedData = try nodeKeys.decryptEnvelope(encryptedData)
    ...
    func testDecryptWithProfileInvalidEnvelopeData() throws {
        ...
        XCTAssertThrowsError(try nodeKeys.decryptEnvelope(invalidEnvelope)) { error in
    ...
    func testDecryptWithProfileEmptyEnvelopeData() throws {
        ...
        XCTAssertThrowsError(try nodeKeys.decryptEnvelope(emptyEnvelope)) { error in
    ...
    func testDecryptWithProfileWrongManagerType() throws {
        // Test calling node function on mobile handle
        let label = "decrypt-test"
        let profileKey = try mobileKeys.mobileDeriveUserProfileKey(label: label)
        let testData = "Hello, Profile Key!".data(using: .utf8)!
        let encryptedData = try mobileKeys.mobileEncryptWithEnvelope(plaintext: testData, profileKeys: [profileKey])
        
        XCTAssertThrowsError(try nodeKeys.decryptEnvelope(encryptedData)) { error in
            XCTAssertTrue(error is FFIError, "Should throw FFIError")
        }
    }
    ...
    func testDecryptWithProfileNotInitialized() throws {
        ...
        XCTAssertThrowsError(try uninitializedKeys.decryptEnvelope(testEnvelope)) { error in
            XCTAssertTrue(error is FFIError, "Should throw FFIError")
        }
    }
    ...
    func testProfileKeyWorkflowMultipleLabels() throws {
        ...
        let encryptedData = try nodeKeys.encryptWithEnvelope(plaintext: testData, profileKeys: profileKeys)
    ...
    func testProfileKeyErrorHandlingConsistency() throws {
        ...
        let retrievedKey = try nodeKeys.getProfilePublicKeyByLabel(nonExistentLabel)
    ...
    func testProfileKeyMemoryManagement() throws {
        ...
        let newKeys = try KeysHandle()
        try newKeys.initializeAsNode()
        try newKeys.nodeGenerateKeys()
        
        // New handle should not have previous profile keys
        let retrievedKey = try newKeys.getProfilePublicKeyByLabel(labels[0])
